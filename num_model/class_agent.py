...
class flock:
    ...
    def __init__(self, dim, n_a, n_c, epsilon, r, d, h=1, c1=0.01, c2=0.3, a=0.05, b=0.05) -> None:
        self.dimensions = dim
        self.nb_agents = n_a
        self.eps = epsilon
        self.r = r
    ...
    def update_a_agent(self, dt):
        ...
        for i,a_a1 in enumerate(self.a_agents):
            a_a1.pi += dt * a_a1.fi(a_a1.qi, c_a.qr, a_a1.pi, c_a.pr)
    ...
    def compute_vel_missmatch(self):
        pr = self.c_agents[0].pr
        return 1/(2*self.nb_agents) * np.sum([fnc.norm_2(ai.pi-pr)**2 for ai in self.a_agents])
